...
%token <ident> T_PRIVATE_SET   327   "'private(set)'"
%token <ident> T_PROTECTED_SET   328 "'protected(set)'"
%token <ident> T_PUBLIC_SET   329    "'public(set)'"
%token <ident> T_READONLY   330      "'readonly'"
%token <ident> T_VAR   331           "'var'"
%token <ident> T_UNSET   332         "'unset'"
%token <ident> T_ISSET   333         "'isset'"
%token <ident> T_EMPTY   334         "'empty'"
%token <ident> T_HALT_COMPILER   335 "'__halt_compiler'"
%token <ident> T_CLASS   336         "'class'"
%token <ident> T_TRAIT   337         "'trait'"
%token <ident> T_INTERFACE   338     "'interface'"
%token <ident> T_ENUM   339          "'enum'"
%token <ident> T_EXTENDS   340       "'extends'"
%token <ident> T_IMPLEMENTS   341    "'implements'"
%token <ident> T_NAMESPACE   342     "'namespace'"
%token <ident> T_LIST   343            "'list'"
%token <ident> T_ARRAY   344           "'array'"
%token <ident> T_CALLABLE   345        "'callable'"
%token <ident> T_LINE   346            "'__LINE__'"
%token <ident> T_FILE   347            "'__FILE__'"
%token <ident> T_DIR   348             "'__DIR__'"
%token <ident> T_CLASS_C   349         "'__CLASS__'"
%token <ident> T_TRAIT_C   350         "'__TRAIT__'"
%token <ident> T_METHOD_C   351        "'__METHOD__'"
%token <ident> T_FUNC_C   352          "'__FUNCTION__'"
%token <ident> T_PROPERTY_C   353      "'__PROPERTY__'"
%token <ident> T_NS_C   354            "'__NAMESPACE__'"
...
%token T_INC   379 "'++'"
%token T_DEC   380 "'--'"
%token T_INT_CAST   381    "'(int)'"
%token T_DOUBLE_CAST   382 "'(double)'"
%token T_STRING_CAST   383 "'(string)'"
%token T_ARRAY_CAST   384  "'(array)'"
%token T_OBJECT_CAST   385 "'(object)'"
%token T_BOOL_CAST   386   "'(bool)'"
%token T_UNSET_CAST   387  "'(unset)'"
%token T_OBJECT_OPERATOR   388 "'->'"
%token T_NULLSAFE_OBJECT_OPERATOR   389 "'?->'"
%token T_DOUBLE_ARROW   390    "'=>'"
%token T_COMMENT   391         "comment"
%token T_DOC_COMMENT   392     "doc comment"
%token T_OPEN_TAG   393        "open tag"
%token T_OPEN_TAG_WITH_ECHO   394 "'<?='"
%token T_CLOSE_TAG   395       "'?>'"
%token T_WHITESPACE   396      "whitespace"
%token T_START_HEREDOC   397   "heredoc start"
%token T_END_HEREDOC   398     "heredoc end"
...
%token T_AMPERSAND_NOT_FOLLOWED_BY_VAR_OR_VARARG   408 "amp"
%token T_BAD_CHARACTER   409   "invalid character"
